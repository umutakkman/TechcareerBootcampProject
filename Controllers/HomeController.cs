...
public class HomeController : Controller
{
        ...
        int page = 1, int pageSize = 6)
    {
        ...
        if (minPrice > 0) cars = cars.Where(c => c.RentPrice >= minPrice).ToList();
        if (maxPrice > 0) cars = cars.Where(c => c.RentPrice <= maxPrice).ToList();

        cars = cars.Where(c => c.IsActive).ToList();
    ...
    {
        if (model.CarId <= 0) return RedirectToAction("Index");
